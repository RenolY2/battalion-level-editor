...
from plugins.bw_texture_conv.lib.read_binary import *
from plugins.bw_texture_conv.lib.texture_utils import *
...
class Texture(object):
    ...
    def dump_to_file(self, filepath):
        img = QImage(self.size_x, self.size_y, QImage.Format.Format_ARGB32)
        rgbadata = self.rgba
...
class BW2Texture(Texture):
    ...
    @classmethod 
    def from_file(cls, f, no_decode=False):
        #f.seek(0)
        start = f.tell()
        ...
        assert tex.unkint2 in (4, 4100, 4108, 4116), str(tex.unkint2)
...
class BW1Texture(Texture):
    ...
    def write(self, f):
        start = f.tell()
        print(self.name, len(self.name))
        assert len(self.name) <= 0x10, "Texture name too long"
        f.write(self.name.encode("ascii"))
