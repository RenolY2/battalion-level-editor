...
class PlanarQuad(object):
    def __init__(self, p1, p2, p3, p4):
        ...
        self.normal = self.p1_to_p2.cross(p4-p1)
...
class Line(object):
    ...
    def collide_quad(self, quad: Quad, d_filter=inf):
        ...
        d = ((quad.origin - self.origin).dot(normal)) / normal.dot(self.direction)
        ...
        C0 = intersection_point - quad.origin

        if quad.normal.dot(quad.p1_to_p2.cross(C0)) > 0:
            C1 = intersection_point - quad.p2

            if quad.normal.dot(quad.p2_to_p4.cross(C1)) > 0:
                C2 = intersection_point - quad.p4

                if quad.normal.dot(quad.p4_to_p3.cross(C2)) > 0:
                    C3 = intersection_point - quad.p3

                    if quad.normal.dot(quad.p3_to_p1.cross(C3)) > 0:
